...
function EditAddress() {
  ...
  const [longitude, setLongitude] = useState(selectedAddress?.longitude);
  const [latitude, setLatitude] = useState(selectedAddress?.latitude);
  const [postalCode, setPostalCode] = useState(selectedAddress?.postal_code);
  const [defaultAddress, setDefaultAddress] = useState(
    selectedAddress.id == defaultAddressID ? "yes" : "no"
  );
  ...
  async function updateAddress(e) {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_NEW_API_BASE}/update-address`,
        {
          method: "POST",
  ...
  return (
    <form onSubmit={updateAddress} className=" w-full">
      <p className="text-lg mt-3 lg:text-[2rem] font-medium text-center">
        Update Address
      </p>
      ...
      <div className="mt-5 flex items-center justify-center w-full">
        <div className="lg:w-[80%] w-[98%] flex justify-between items-center gap-x-5">
          ...
          >
            {" "}
            <Icon icon="mdi:location" className="w-5 h-5 text-white" /> Update
            Address
          </button>
        </div>
      </div>
