...
function BidList() {
    ...
    async function fetchData() {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_NEW_API_BASE}/past-bids`,
          {
            headers: {
        ...
        // const past_bids = data.bids.filter((bid) => bid.past_bid === true);
        setPastBids(data.bids);
        // console.log(data.bids);
        setDuplicatedPastBids(data.bids);
  ...
  const currentItems = pastBids.slice(offset, endOfOffset);
  const pageCount = Math.ceil(pastBids.length / itemsPerPage);
  ...
  const handlePageClick = (event) => {
    ...
    const newOffset = (event.selected * itemsPerPage) % pastBids.length;
    console.log(
  ...
  const endIndex = Math.min((currentPage + 1) * itemsPerPage, pastBids.length);
  ...
  return (
    <div ref={listContainerRef} className="mt-8 w-full scroll-mt-56">
      <div className="w-full">
          ...
          <div className="flex w-full lg:w-[60%] xl:w-[50%] justify-center lg:justify-end">
            <div className="flex items-center gap-x-3">
              ...
              <p className=" font-medium whitespace-nowrap">
                ...
                {pastBids.length === 0 ? 0 : startIndex} to {endIndex} of {pastBids.length} Item(s)
                </span>
              </p>
            </div>
          </div>
        ...
        {fetching && pastBids.length === 0 && (
          <p className="mt-10">Loading bids... Please wait.</p>
        )}

        {!fetching && pastBids.length === 0 && (
          <p className="text-red-600 mt-10">No past bids to display.</p>
        )}
