...
function ClaimItem() {
  ...
  async function makePayment(e) {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_NEW_API_BASE}/make-payment`,
        {
          method: "POST",
          body: JSON.stringify(dataToSubmit),
          headers: {
    ...
    <div className="mt-5 xl:flex  gap-5 justify-between w-full h-full  ">
      ...
      <div className="w-full xl:w-1/2 mt-5 xl:mt-0">
        <div className=" border rounded p-4">
          <p className="font-semibold text-2xl">Item successfully paid for.</p>
          ...
          {/* <div className="flex items-center justify-between gap-x-5 mt-3">
            ...
            <p className="text-lg font-medium">
              ${parseFloat(selectedBid?.buyers_premium).toFixed(2).toLocaleString()}
            </p>
          </div> */}
          ...
          <div className="flex items-center justify-between gap-x-5 mt-3">
            ...
            <p className="text-lg font-medium">
              ${parseFloat(+selectedBid?.bid_tax).toFixed(2).toLocaleString()}
            </p>
          </div>
            ...
            {!selectedBid?.bid_payment &&
              ((grandTotal = parseFloat(
                +selectedBid?.current_bid +
                  ...
                  // +selectedBid?.buyers_premium +
                  +selectedBid?.bid_tax +
                  (deliveryType === "doorstep" ? +selectedBid?.shipping_fee : 0)
              )
                .toFixed(2)
                .toLocaleString()),
              (
