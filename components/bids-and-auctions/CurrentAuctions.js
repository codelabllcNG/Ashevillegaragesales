...
function CurrentAuctions({ fetching }) {
  ...
  const currentItems = currentAuctions.slice(offset, endOfOffset);
  const pageCount = Math.ceil(currentAuctions.length / itemsPerPage);
  ...
  const handlePageClick = (event) => {
    ...
    const newOffset = (event.selected * itemsPerPage) % currentAuctions.length;
    console.log(
  ...
  return (
    <div ref={listContainerRef} className="mt-14 scroll-mt-60">
      {fetching && currentAuctions.length === 0 && (
        <p>Loading auctions... Please wait.</p>
      )}
      {!fetching && currentAuctions.length === 0 && (
        <p className="text-red-600">
          No Current Auctions at this time, check other categories.
        </p>
      )}
