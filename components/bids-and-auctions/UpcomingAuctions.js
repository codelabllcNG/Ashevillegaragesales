...
function UpcomingAuctions({ fetching }) {
  ...
  const currentItems = upcomingAuctions.slice(offset, endOfOffset);
  const pageCount = Math.ceil(upcomingAuctions.length / itemsPerPage);
  ...
  const handlePageClick = (event) => {
    ...
    const newOffset = (event.selected * itemsPerPage) % upcomingAuctions.length;
    console.log(
  ...
  return (
    <div ref={listContainerRef} className="mt-14 scroll-mt-60">
      {fetching && upcomingAuctions.length === 0 && (
        <p>Loading auctions... Please wait.</p>
      )}
      {!fetching && upcomingAuctions.length === 0 && (
        <p className="text-red-600">
          No Upcoming Auctions at this time, check other categories.
        </p>
      )}
